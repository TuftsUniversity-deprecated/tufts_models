module AttachedFiles
  ...
  def remote_url_for(name, extension)
      ...
      if self.datastreams[name].nil? || self.datastreams[name].dsLocation.nil?
  ...
  def local_path_for(name, extension=nil)
    ...
    if self.datastreams[name].nil? || self.datastreams[name].dsLocation.nil?
      file_path = File.join(local_path_root, file_path(name, extension))
    else
      file_path = (self.datastreams[name].dsLocation).gsub('http://bucket01.lib.tufts.edu',Settings.object_store_root)
    end
  ...
  def directory_for(name)
    if name.downcase.eql? 'thumbnail.png'
      name = 'thumb.png'
    end
