Tufts::Application.routes.draw do
  ...
  resources :catalog, :only => [:show, :update], :constraints => { :id => /[a-zA-Z0-9_.:]+/, :format => false }
  ...
  resources :downloads, :only =>[:show], :constraints => { :id => /[a-zA-Z0-9_.:]+/ }

  
  HydraHead.add_routes(self)
  mount FcrepoAdmin::Engine => '/admin', :as=> 'fcrepo_admin'
  mount HydraEditor::Engine => '/'
  post 'records/:id/publish', to: 'records#publish', as: 'publish_record', constraints: { id: /[a-zA-Z0-9_.:]+/ }

  resources :records, only: [] do
    resources :attachments, constraints: { id: /[a-zA-Z0-9_.:]+/ }
  end
